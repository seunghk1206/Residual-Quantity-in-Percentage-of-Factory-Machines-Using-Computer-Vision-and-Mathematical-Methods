...
def PhotoAnalysisDark(Dir, actualHeight, actualSideLength, cubicThreshold):#만약 깔때기에 넣는 물체가 깔때기의 색보다 어두울 경우에 쓰는 함수입니다.
    ...
    if percentageBlue > cubicThreshold:
        ...
        return 1-((percentageHeight*actualHeight*actualSideLength**2+pyramidalVol)/totalVolume)
    else:
        percentagePyramidal = (percentageBlue/cubicThreshold)
        # 2a^2+b^2 = c^2 --> k(2a^2+b^2)^(1/2)= k c #a^2*b
        return 1-(pyramidalVol*percentagePyramidal/totalVolume)